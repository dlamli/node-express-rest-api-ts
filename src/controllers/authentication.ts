...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    if (user.authentication.password !== expectedHash.toString()) return res.sendStatus(403)
    ...
    res.cookie('AUTH-LOGIN', user.authentication.sessionToken, { domain: 'localhost', path: '/' })
